...
export const config = {
  ...
  startupStrict: (process.env.DA_STARTUP_STRICT || "true").trim().toLowerCase() === "true",
  startupChainRpc: (process.env.DA_CHAIN_RPC || "").trim(),
  startupExpectedChainId: (process.env.DA_EXPECTED_CHAIN_ID || "").trim(),
  startupEntranceContractAddr: (process.env.DA_ENTRANCE_CONTRACT_ADDR || "").trim(),
  startupCheckTimeoutMs: Math.max(asInt(process.env.DA_STARTUP_CHECK_TIMEOUT_MS, 8000), 500),
